...
(function ($, $$) {
	...
	var _ = self.Mavo = $.Class({
		constructor: function constructor(element) {
			...
			$$(_.selectors.primitive + "," + _.selectors.multiple, this.element).forEach(function (element) {
				if ($$(_.selectors.property, element).length) {
   ...
   */
		...
		setDeleted: function setDeleted() {
			...
			(_deleted = this.deleted).push.apply(_deleted, nodes);
			...
			var notice = this.deletionNotice = this.message([this._("item-deleted", { name: phrase }), {
				tag: "button",
				textContent: this._("undo"),
				events: {
					click: function click(evt) {
						_this7.undoDelete();
						_this7.deletionNotice.close(true);
					}
				}
			}], {
				classes: "mv-deleted",
			...
			notice.closed.then(function (undone) {
				if (!undone && _this7.deleted.length) {
					// Gone forever now
					_this7.deleted.forEach(function (node) {
						return node.destroy();
					});
					_this7.deleted.length = 0;
				}

				if (_this7.deletionNotice == notice) {
					_this7.deletionNotice = null;
				}
			});
		},
...
(function ($, $$) {
   ...
   */
		...
		objectify: function objectify(value, properties) {
			...
			if ((typeof value === "undefined" ? "undefined" : _typeof(value)) !== "object" || value === null) {
				...
				value.valueOf = value.toJSON = value[Symbol.toPrimitive] = function () {
					return primitive;
				};
			}
   ...
   */
		BucketMap: function (_Map) {
			_inherits(BucketMap, _Map);

			function BucketMap() {
				var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
				    _ref$arrays = _ref.arrays,
				    arrays = _ref$arrays === undefined ? false : _ref$arrays;

				_classCallCheck(this, BucketMap);

				var _this2 = _possibleConstructorReturn(this, (BucketMap.__proto__ || Object.getPrototypeOf(BucketMap)).call(this));

				_this2.arrays = arrays;
				return _this2;
			}
			...
			_createClass(BucketMap, [{
				...
				value: function set(key, value) {
					...
					_get(BucketMap.prototype.__proto__ || Object.getPrototypeOf(BucketMap.prototype), "set", this).call(this, key, values);
				}
			}, {
				key: "delete",
				value: function _delete(key, value) {
					if (arguments.length == 2) {
						...
						_get(BucketMap.prototype.__proto__ || Object.getPrototypeOf(BucketMap.prototype), "delete", this).call(this, key);
					}
				}
			}]);
	...
	var _ = Mavo.UI.Message = $.Class({
		constructor: function constructor(mavo, message) {
			...
			if (o.classes) {
				...
				(_element$classList = this.element.classList).add.apply(_element$classList, _toConsumableArray(o.classes.split(/\s+/)));
			}
...
(function ($, $$) {
   ...
   */
		...
		updateParentLiveData: function updateParentLiveData(value) {
			if (value === undefined) {
   ...
   */
		...
		resolveAgainstData: function resolveAgainstData(property, data) {
			if (property in data) {
		...
		relativizeData: self.Proxy ? function (data) {
			...
			return new Proxy(data, {
				get: function get(data, property, proxy) {
					...
					return _this3.resolveAgainstData(property, data);
				},

				has: function has(data, property) {
					var ret = _this3.resolveAgainstData(property, data);
    ...
    */
			...
			get: function get(element, prioritizePrimitive) {
				var nodes = (_.all.get(element) || []).filter(function (node) {
					return !/Collection$/.test(node.nodeType);
				});
				...
				if (isCollection === "multiple") {
					...
					if (existing instanceof Mavo.Collection) {
						...
						(existing || []).forEach(function (e, i) {
							return _this.children[property].add(e, i);
						});
					} else {
						_this.children[property] = [].concat(_toConsumableArray(existing || []), [element]);
					}
				} else if (isCollection > 1) {
					if (!_this.children[property]) {
						_this.children[property] = new Mavo.ImplicitCollection(element, _this.mavo, options);
					} else {
						_this.children[property].add(element);
					}
				} else {
					// Normal case
...
(function ($, $$) {
    ...
    */
		...
		updateLiveData: function updateLiveData() {
		...
		getLiveData: function getLiveData() {
			...
			if (this.liveData && _typeof(this.liveData) === "object") {
			...
			if (!this.popup && this.closestCollection && this.editor.matches(Mavo.selectors.textInput)) {
				this.editor.addEventListener("keydown", function (evt) {
					if (evt.keyCode == 13 && _this2.closestCollection.editing && (evt.shiftKey || !multiline)) {
						...
						var closestItem = _this2.closestItem;
						var next = _this2.closestCollection.add(undefined, closestItem && closestItem.index + 1);
		...
		edit: function edit() {
			...
			this.preEdit = Mavo.promise(function (resolve) {
				if (o.immediately) {
					return resolve();
				}
			...
			return this.preEdit.then(function () {
				_this3.element.classList.remove("mv-pending-edit");
				...
				_this3.sneak(function () {
					...
					if (!_this3.attribute && !_this3.popup) {
						if (_this3.editor.parentNode != _this3.element) {
							...
							if (_this3.config.hasChildren) {
								_this3.element.textContent = "";
							} else {
								_.setText(_this3.element, "");
							}

							_this3.element.prepend(_this3.editor);
						}
		...
		dataRender: function dataRender(data, live) {
					...
					var upload = function upload(file) {
						...
						_this.mavo.upload(file, path + "/" + name).then(function (url) {
							...
							var load = Mavo.rr(function () {
								return Mavo.defer(1000 + attempts * 500).then(function () {
									attempts++;
		...
		"time": {
			...
			defaultFormats: {
				...
				"time": function time(name) {
					return "[digits(2, hour(" + name + "))]:[digits(2, minute(" + name + "))]";
				},
...
(function ($, $$) {
   ...
   */
		...
		delete: function _delete(item) {
			...
			var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
			    silent = _ref.silent,
			    _ref$undoable = _ref.undoable,
			    undoable = _ref$undoable === undefined ? !silent : _ref$undoable,
			    _ref$transition = _ref.transition,
			    transition = _ref$transition === undefined ? !silent : _ref$transition;
   ...
   */
		...
		save: function save() {
			this.children.forEach(function (item) {
				return item.unsavedChanges = false;
			});
		},
			...
			addButton: function addButton() {
				...
				if (group) {
					var button = $$(selector, group).filter(function (button) {
						return !_this8.templateElement.contains(button) // is outside the template element
		...
		static: {
			...
			delete: function _delete(nodes) {
				...
				var promises = nodes.filter(function (node) {
					...
					return node.collection.delete(node, Object.assign({}, o, { undoable: false })).then(function (node) {
...
(function ($, $$) {
	...
	var _ = Mavo.DOMExpression = $.Class({
		constructor: function constructor() {
			...
			var _arr = ["item", "path", "syntax", "fallback", "attribute", "originalAttribute", "expression", "parsed", "identifiers"];
			for (var _i = 0; _i < _arr.length; _i++) {
				var prop = _arr[_i];
			...
			this.mavo.treeBuilt.then(function () {
				...
				_this.mavo.expressions.register(_this);
	...
	var _ = Mavo.Expressions = $.Class({
		...
		register: function register(domexpression) {
			...
			domexpression.identifiers.forEach(function (id) {
				...
				if (Mavo.all[id] instanceof Mavo && Mavo.all[id] !== _this2.mavo) {
		...
		updateByIdThrottled: function updateByIdThrottled(property, evt, cache) {
			...
			if (property.forEach) {
				property.forEach(function (property) {
					return _this5.updateByIdThrottled(property, evt, cache);
				});
			} else {
				...
				if (!scheduled.has(property)) {
					setTimeout(function () {
						...
						_this5.updateById(property, evt, cache);
					}, _.THROTTLE);
		...
		traverse: function traverse(node) {
			...
			if (node.nodeType === 3) {
				...
				$$(node.attributes).forEach(function (attribute) {
					return _this6.extract(node, attribute, path, syntax);
				});
				...
				if (!node.matches("script:not([mv-expressions])")) {
					$$(node.childNodes).forEach(function (child) {
						...
						if (child.nodeType == 1 || child.nodeType == 3) {
							...
							_this6.traverse(child, [].concat(_toConsumableArray(path || []), [segment]), syntax);
						}
					});
				}
			}
...
(function ($, val) {
   ...
   */
		...
		digits: function digits(_digits, decimals, num) {
			if (num === undefined) {
				num = decimals;
				decimals = undefined;
			}
   ...
   */
	...
	_._Trap = self.Proxy ? new Proxy(_, {
		get: function get(functions, property) {
			...
			if ((typeof property === "undefined" ? "undefined" : _typeof(property)) === "symbol") {
				return;
			}
...
(function ($, val, _) {
	...
	$.extend(_, {
		...
		date: function date(_date) {
			...
			return _date ? _.year(_date) + "-" + _.digits(2, _.month(_date)) + "-" + _.digits(2, _.day(_date)) : "";
		},
	...
	var _loop = function _loop(what) {
		_[what] = function (ms) {
			return _.msTo(what, ms);
		};
	};
	...
	$.extend(_.util, {
		...
		date: function date(_date2) {
			...
			return isNaN(_date2) ? null : _date2;
		}
	});
...
(function ($, val, $u) {
		...
		operators: {
			...
			"object": {
				...
				code: function code() {
					...
					var value = arguments.length <= i ? undefined : arguments[i];

					while (i--) {
						value = _defineProperty({}, arguments.length <= i ? undefined : arguments[i], value);
					}
   ...
   */
		...
		serialize: function serialize(node) {
			...
			if ((typeof ret === "undefined" ? "undefined" : _typeof(ret)) == "object" && ret && ret.type) {
				node = ret;
...
(function ($, $$) {
    ...
    */
			add: function add(ref, data, index) {
				if (!ref) {
					return;
				}
    ...
    */
			clear: function clear() {
				for (var _len = arguments.length, ref = Array(_len), _key = 0; _key < _len; _key++) {
					ref[_key] = arguments[_key];
				}
	...
	var _loop = function _loop(name) {
		...
		if (!(nameif in _.Functions)) {
			_.Functions[nameif] = function (condition, target) {
				...
				return (_$Functions2 = _.Functions)[name].apply(_$Functions2, [target].concat(rest));
			};
		}
	};
